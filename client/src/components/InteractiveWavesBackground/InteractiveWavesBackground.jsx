  ...
  useEffect(() => {
    ...
    const setSize = (entry) => {
      ...
      boundingRef.current = {
        width: rect.width,
        height: rect.height,
        left: entry ? container.getBoundingClientRect().left : rect.left,
        top: entry ? container.getBoundingClientRect().top : rect.top
      };
    ...
    const setLines = () => {
      ...
      linesRef.current = [];
      const oWidth = width + 200, oHeight = height + 150;
    ...
    const movePoints = (time) => {
      ...
      lines.forEach(pts => {
        pts.forEach(p => {
          ...
          const l = Math.max(280, mouse.vs * 2.5);
    ...
    const tick = (t) => {
      ...
      mouse.vs += (Math.min(100, d) - mouse.vs) * 0.1;
...
export default React.memo(InteractiveWavesBackground);
